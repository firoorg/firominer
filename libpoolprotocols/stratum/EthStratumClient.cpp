...
{
    return ParseUInt32(value, &result, hexadecimal ? 16 : 10);
}
...
void EthStratumClient::connect()
{
        ...
        m_conn->HostNameType() == dev::UriHostNameType::Basic)
    {
        ...
        m_resolver.async_resolve(
            m_conn->Host(), toString(m_conn->Port()), tcp::resolver::address_configured,
            m_io_strand.wrap(guarded(&EthStratumClient::resolve_handler)));
    }
    else
    {
        ...
        boost::asio::post(m_io_service, m_io_strand.wrap(guarded(&EthStratumClient::start_connect)));
    }
}
...
void EthStratumClient::disconnect()
{
    ...
    m_resolver.cancel();
    // Autodetection can have a reconnect queued after the old socket was released.
    if (!m_socketState && !m_session && m_conn)
        disconnect_finalize();
    else
        disconnectInternal();
}

void EthStratumClient::disconnectInternal()
{
    std::lock_guard<std::recursive_mutex> callbackLock(m_callbackState->mutex);
...
void EthStratumClient::disconnect_finalize()
{
    ...
    if (m_connectionWanted && m_conn && !m_conn->IsUnrecoverable())
    {
        ...
        if (isConnected())
        {
            ...
            if (response_delay_ms.count() >= (m_responsetimeout * 1000))
            {
                ...
                if (!m_conn->StratumModeConfirmed() && !m_conn->IsUnrecoverable())
                {
                    ...
                    jRes["error"] = true;
                    clear_response_pleas();
                    boost::asio::post(m_io_service, m_io_strand.wrap(guarded(
...
void EthStratumClient::handshake_handler(const boost::system::error_code& ec)
{
    ...
    if (ec)
    {
        ...
        const bool sslError = ec.category() == boost::asio::error::get_ssl_category();
        const auto reason = sslError ? ERR_GET_REASON(ec.value()) : 0;
        if (sslError && reason == SSL_R_CERTIFICATE_VERIFY_FAILED)
        ...
        if (sslError && ec != boost::asio::ssl::error::stream_truncated
    ...
    std::string const& enonce, unsigned maxBytes, bool allowEmpty)
{
    ...
    */
    ...
    try
    {
        ...
        {
            if (enonce.size() > maxBytes * 2 + 2)
                throw std::invalid_argument("Extranonce exceeds allowed width");
            hexPart = enonce.compare(0, 2, "0x") == 0 ? enonce.substr(2) : enonce;
            if (hexPart.empty() || !IsHex(hexPart))
                throw std::invalid_argument("Invalid hex value");
            if (hexPart.length() & 1)
                throw std::invalid_argument("Odd number of hex chars");
        ...
        if (_id == 1)
        {
            ...
            */
            ...
            case EthStratumClient::ETHEREUMSTRATUM2:
                ...
                _isSuccess = (_isSuccess && jResult.isObject() && jResult.isMember("proto") &&
            ...
            case EthStratumClient::ETHEREUMSTRATUM:
                ...
                _isSuccess = (_isSuccess && jResult.isArray() && jResult[0].isArray() && jResult[0].size() == 3 &&
    ...
    if (!ec)
    {
        ...
        */
        ...
        std::string rx_message(
            static_cast<const char*>(recvBuffer.data().data()), bytes_transferred);
        ...
        while (offset != string::npos)
        {
            ...
            if (offset > 0)
            {
                ...
                if (!line.empty())
                {
                    ...
                    {
                        cwarn << "Stratum got invalid Json message : " << ex.what();
                    }
                }
            }
...
void EthStratumClient::onSSLShutdownCompleted(const boost::system::error_code& ec)
{
    (void)ec;
    clear_response_pleas();
    boost::asio::post(m_io_service, m_io_strand.wrap(guarded(&EthStratumClient::disconnect_finalize)));
}
